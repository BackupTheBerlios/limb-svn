...
//very ugly and messy class, totally refactored in 3.0
class image_object extends media_object
{
  ...
  function _create_generate_operation($variation, &$result)
  {
    ...
    if(!isset($files_data[$this->get_attribute($variation . '_base_variation')]['name']))
    {
    ...
    $name_parts = explode('.', $files_data[$this->get_attribute($variation . '_base_variation')]['name']);
  ...
  function _create_upload_operation($variation, &$result)
  {
    ...
    if(	!isset($files_data[$variation]['name']) ||
        !isset($files_data[$variation]['tmp_name']) ||
        !isset($files_data[$variation]['type']))
    {
      debug :: write_error('uploaded file not found',
        __FILE__ . ' : ' . __LINE__ . ' : ' .  __FUNCTION__,
        array('variation' => $variation));
    ...
    $this->_insert_variation(
      $variation,
      $files_data[$variation]['tmp_name'],
      $files_data[$variation]['name'],
      $files_data[$variation]['type']
    );
