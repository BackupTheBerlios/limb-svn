 ...
 *
 *   $Id: groupcp.php,v 1.1 2003/07/04 18:37:45 pacha Exp $
 *
 *
 ***************************************************************************/
...
if ( isset($HTTP_POST_VARS['groupstatus']) && $group_id )
{
	...
	{
		header($header_location . append_sid("login.php?redirect=groupcp.php&" . POST_GROUPS_URL . "=$group_id", true));
		exit;
	}
...
else if ( isset($HTTP_POST_VARS['unsub']) || isset($HTTP_POST_VARS['unsubpending']) && $group_id )
{
	...
	if ( $cancel )
	{
		header($header_location . append_sid("groupcp.php", true));
		exit;
	}
	...
	if ( $confirm )
	{
		...
		$message = $lang['Usub_success'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . append_sid("groupcp.php?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid("index.php") . '">', '</a>');
