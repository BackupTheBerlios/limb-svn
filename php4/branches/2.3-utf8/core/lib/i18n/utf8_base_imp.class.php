...
class utf8_base_imp
{
  ...
  {
    if($start < 0)
      $start = $this->utf8_strlen($str) + $start;

    if($length < 0)
      $length = $this->utf8_strlen($str) + $length - $start;
  ...
  function  utf8_trim($str,$charlist='')
  {
    ...
    return $this->utf8_ltrim($this->utf8_rtrim($str, $charlist), $charlist);
  }
  ...
  function utf8_stripspecials($string,$repl='',$keep='')
  {
    ...
    if($keep != '')
      $specials = array_diff($UTF8_SPECIAL_CHARS, $this->utf8_to_unicode($keep));
    else
      $specials = $UTF8_SPECIAL_CHARS;

    $specials = $this->unicode_to_utf8($specials);
  ...
  function utf8_strpos($haystack, $needle, $offset=0)
  {
    if(!$offset)
    {
      $ar = $this->utf8_explode($needle, $haystack);
      if ( count($ar) > 1 )
    ...
    else
    {
      ...
      if ( false !== ($pos = $this->utf8_strpos($haystack, $needle)))
         return $pos + $offset;
