...
*
***********************************************************************************/
require_once(LIMB_STATS_DIR . '/DAO/StatsSearchEnginesReportDAO.class.php');

class StatsSerachEnginesReportDAOTest extends LimbTestCase
{
  function StatsSerachEnginesReportDAOTest()
  {
    parent :: LimbTestCase('stats search engines report DAO test');
  }
  ...
  function testFetch()
  {
    ...
    $dao = new StatsSearchEnginesReportDAO();
    $rs =& $dao->fetch();
    ...
    $record =& $rs->current();
    $this->assertEqual($record->get('engine'), $engine2);
    $this->assertEqual($record->get('hits'), 3);

    $rs->next();
    $record =& $rs->current();
    $this->assertEqual($record->get('engine'), $engine1);
