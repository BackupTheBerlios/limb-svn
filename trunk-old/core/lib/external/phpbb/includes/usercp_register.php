	...
	$mode == 'register' )
{
	...
	$strip_var_list = array('icq' => 'icq', 'aim' => 'aim', 'msn' => 'msn', 'yim' => 'yim', 'website' => 'website', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests');
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( !$error )
	{
		...
		if ( $mode == 'editprofile' )
		{
			if ( $board_config['require_activation'] != USER_ACTIVATION_NONE && $userdata['user_level'] != ADMIN )
			{
				$user_active = 0;
...
else if ( $mode == 'editprofile' && !isset($HTTP_POST_VARS['avatargallery']) && !isset($HTTP_POST_VARS['submitavatar']) && !isset($HTTP_POST_VARS['cancelavatar']) )
{
	...
	$current_email = $userdata['user_email'];
	$new_password = '';
	$password_confirm = '';
