...
class db_peer
{
	...
	*/
	function do_delete($criteria)
	{
		if (! is_a($criteria, 'criteria'))
			return new exception (LIMB_ERROR, "parameter 1 not of type 'criteria' !");

		$connection= &db_factory::get_connection($criteria->get_connection_name());
		$db_map = &db_factory::get_database_map($criteria->get_connection_name());
		...
		foreach($tables as $table_name)
		{
			...
			if ($criteria->is_single_record())
			{
				...
				if (is_error($rs))
				{
					//$logger->err($rs->get_message());
					return new exception(LIMB_ERROR_DB, "Unable to execute DELETE statement.", $rs);
				} 
				$rs->next();
				if ($rs->get_int(1) > 1)
				{
					$rs->close();
					return new exception(LIMB_ERROR, "Expecting to delete 1 record, but criteria match multiple.");
				} 
				$rs->close();
			} 
			...
			if (is_error($result))
			{
				//$logger->err($result->get_message());
				return new exception(LIMB_ERROR_DB, "Unable to execute DELETE statement.", $result);
			} 
		} // for each table
		return $result;
	} 
	...
	*/
	function do_insert($criteria, &$con)
	{
		...
		if (empty($keys))
		{
			return new exception(LIMB_ERROR, "Database insert attempted without anything specified to insert");
		} 
		...
		{ 
			...
			if ($key_gen->is_before_insert())
			{
				...
				if (is_error($id))
				{
					return new exception(LIMB_ERROR, "Unable to get sequence id.", $id);
				} 
		...
		{
			//$logger->err($e->get_message());
			return new exception(LIMB_ERROR_DB, "Unable to execute INSERT statement.", $e);
		} 
		...
		if ($pk !== null && $use_id_gen && $key_gen->is_after_insert())
		{
			...
			if (is_error($id))
			{
				//$logger->err("Unable to get autoincrement id: ", $id->get_message());
				return new exception(LIMB_ERROR_DB, "Unable to get autoincrement id.", $id);
			} 
		} 
	...
	*/
	...
	{
		$connection= &db_factory::get_connection($select_criteria->get_connection_name());
		$db_map = &db_factory::get_database_map($select_criteria->get_connection_name());
		//$logger = &db_factory::logger();
		...
		foreach($tables_columns as $table_name => $columns)
		{
			...
			{ 
				...
				if (is_error($rs))
				{
					return new exception(LIMB_ERROR_DB, "Unable to execute UPDATE statement !", $rs);
				} 
				if ($rs)
				{
					$rs->next();
					...
					{
						$rs->close();
						return new exception(LIMB_ERROR, "Expected to update 1 record, multiple matched.");
					} 
					$rs->close();
				} 
			} 
			...
			if (is_error($e = $stmt->execute_update()))
			{
				...
				if ($stmt) $stmt->close();
				//$logger->err($e->get_message());
				return new exception(LIMB_ERROR_DB, "Unable to execute UPDATE statement.", $e);
			} 
	...
	*/
	function &do_select(&$criteria, &$con)
	{
		...
		if (is_error($rs))
		{
			return new exception(LIMB_ERROR_DB, "Unable to execute SELECT statement !", $rs);
		} 
	...
	* 
	* @param string $connectioname The name of the database
	* @param string $tableName The name of the table
	* @param array $columns Array of column names as key and column values as value.
	*/
	function do_validate($db_name, $table_name, $columns)
	{
		$db_map = db_factory::get_database_map($db_name);
		$table_map = $db_map->get_table($table_name);
		$failure_map = array(); // map of validation_failed objects
		foreach($columns as $col_name => $col_value)
		{
			if ($table_map->contains_column($col_name))
			{
				$col = &$table_map->get_column($col_name);
				foreach($col->get_validators() as $validator_map)
				{
					$validator = &db_peer::get_validator($validator_map->get_class());
					if ($validator->is_valid($col_value, $validator_map->get_value()) === false)
					{
						if (!isset($failure_map[$col_name])) // for now we do one validation_failed per column, not per rule
						{
							$failure_map[$col_name] =& new validation_failed($col_name, $validator_map->get_message());
						} 
					} 
				} 
			} 
		} 
		return (!empty($failure_map) ? $failure_map : true);
	} 
	...
	{ 
		...
		if (!empty($table))
		{
			...
			if ($db_map->get_table($table) == null)
				return new exception(LIMB_ERROR, "\$connectionap->getTable() is null");
	...
	* @private static
	*/
	function build_params($columns,/*criteria*/ $values)
	{
		$params = array();
	...
	function populate_stmt_values(&$stmt, &$params, &$db_map)
	{
		...
		foreach($params as $param)
		{
			...
			else
			{
				...
				if (is_error($setter))
				{
					return new exception(LIMB_ERROR_DB, $setter);
				} 
	...
	* @return mixed Validator object or db_factoryException if not able to instantiate validator class.
	*/
	function &get_validator($classname)
	{
		$self = &db_peer::instance();
		$v = null;

		if (isset($self->validator_map[$classname]))
		{
			$v = &$self->validator_map[$classname];
		} 

		if ($v === null)
		{
			$cls = db_factory::import($classname);
			if (is_error($cls))
			{
				/*$logger = &db_factory::logger();
				$logger->err("BasePeer::getMapBuilder() failed trying to instantiate: " . $classname . ": " . $cls->get_message());*/
				return $cls;
			} 

			$v = &new $cls();
			$self->validator_map[$classname] = &$v;
		} 
