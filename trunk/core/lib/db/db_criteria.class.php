...
* base_peer constructs SQL statements based on the values in this class.
...
class criterion
{
	...
	function &get_connection()
	{
		...
		{ 
			...
			{ 
				...
				/*$logger = &Propel::logger();
				$logger->err("Could not get a DB adapter, so sql may be wrong");*/
			} 
		} 
		else
		{
	...
	*/
	function &add_and(/*criterion*/$criterion)
	{
	...
	* @return criterion 
	*/
	function &add_or(/*criterion*/$criterion)
	{
		$this->clauses[] = &$criterion;
	...
	function append_ps_to(&$sb, &$params)
	{
		...
		else
		{
			...
			{ 
				...
				{
					if (is_a($connection 'DBPostgres')) // use is_a() because instanceof needs class to have been loaded
					{
				...
				if ($this->ignore_string_case && !is_a($connection 'DBPostgres'))
				{
			...
			{ 
				...
				{ 
					...
					{ 
						...
						return new propel_exception(PROPEL_ERROR_SYNTAX, "Could not build SQL for expression: $field " . $this->comparison . " NULL");
					} 
				} 
			} 
		} 
