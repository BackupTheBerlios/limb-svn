...
***********************************************************************************
*
* $Id: mime_mail.class.php 410 2004-02-06 10:46:51Z server $
*
***********************************************************************************/ 
...
{ 
  ...
  var $headers = array('MIME-Version: 1.0'); 
  var $parts = array(); 
  var $charset = 'us-ascii'; 
  
  var $charsetlist = array( 
                            'iso'  => 'us-ascii', 
                            'big5' => 'big5', 
                            'gb'   => 'gb2312' 
                           ); 
  ...
  { 
    $this->do_html = 1; 
    $this->html = $html; 
    $this->html_text = (!$text) ? strip_tags($html) : $text; 
    ...
    { 
        ...
        { 
          ...
          if(($dec == 32) AND ($i == ($linlen - 1)))                // convert space at eol only 
          	$char = '=20'; 
          elseif(($dec == 61) OR ($dec < 32 ) OR ($dec > 126))    // always encode "\t", which is *not* required 
  ...
  { 
	...
	** 
	** $params['html_encoding'] - The type of encoding to use on html. Valid attributes are 
	**                            "7bit", "quoted-printable" or "base64" (all without quotes). 
	**                            7bit is EXPRESSLY NOT RECOMMENDED. Default is quoted-printable 
	** $params['text_encoding'] - The type of encoding to use on plain text Valid attributes are 
	**                            "7bit", "quoted-printable" or "base64" (all without quotes). 
	**                            Default is 7bit 
  ...
  { 
    ...
    $do_html  = (isset($this->do_html) && $this->do_html == 1) ? 1 : 0; 
    ...
    { 
      ...
      if($do_html)  // Build html parts
      	$this->build_html($boundary); 
      elseif($do_text) // Build plain text part 
      	$this->multipart .= '--'.$boundary.CRLF.$this->build_part(array('body' => $this->body, 'name' => '', 'c_type' => 'text/plain')); 
    }
    elseif($do_text && !$do_parts)// No attachments or html, plain text 
    { 
