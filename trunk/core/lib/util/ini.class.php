...
function get_ini_option($file_name, $block_name, $var_name, $use_cache = null)
{
	$ini = &get_ini($file_name, $use_cache);
...
function &get_ini($file_name, $use_cache = null)
{
	...
	if (!($ini = &ini::instance($file_name, $root_dir, $use_cache)))
		error('couldnt retrieve ini instance', __FILE__ . ' : ' . __LINE__ . ' : ' . __FUNCTION__, array('file' => $file_name));
...
{ 
  ...
  */
   ...
   */
	function _parse_file($file)
	{
		...
		return $this->_parse_string($contents);
	} 
	...
	function _parse_string($contents)
	{
		...
		if ($lines === false)
		{
			debug::write_error("'{$this->file_name}' is empty", __FILE__ . ' : ' . __LINE__ . ' : ' . __FUNCTION__);
			return false;
		} 
  ...
  */
	function save($file_name = false, $use_backup = false, $suffix = false, $use_override = false, $only_modified = false)
	{
		...
		$backupfile_name = $originalfile_name . sys::backup_file_name();
		$file_name .= '.tmp';
  ...
  */
	...
	function group($block_name)
	{
		...
		debug::write_notice('unknown block',
			__FILE__ . ' : ' . __LINE__ . ' : ' . __FUNCTION__,
			array('ini' => $this->file_name,
