...
*
*     $Id: admin_smilies.php,v 1.1 2003/07/04 18:37:41 pacha Exp $
*
****************************************************************************/
...
if( isset($HTTP_GET_VARS['import_pack']) || isset($HTTP_POST_VARS['import_pack']) )
{
	...
	if ( !empty($smile_pak) )
	{
		...
		$fcontents = @file($smilies_dir_name . '/'. $smile_pak);
...
else if( isset($HTTP_POST_VARS['add']) || isset($HTTP_GET_VARS['add']) )
{
	...
	$template->assign_vars(array(
		...
		"SMILEY_IMG" => $smilies_dir_name . '/' . $smiley_images[0], 

		"S_SMILEY_ACTION" => append_sid("admin_smilies.$phpEx"), 
		"S_HIDDEN_FIELDS" => $s_hidden_fields, 
		"S_FILENAME_OPTIONS" => $filename_list, 
		"S_SMILEY_BASEDIR" => $smilies_dir_name)
	);
	...
	switch( $mode )
	{
		...
		case 'edit':
			...
			$template->assign_vars(array(
				...
				"SMILEY_IMG" => $smilies_dir_name . '/' . $smiley_edit_img, 
...
else
{
	...
	for($i = 0; $i < count($smilies); $i++)
	{
		...
		$template->assign_block_vars("smiles", array(
			...
			"SMILEY_IMG" =>  $smilies_dir_name . '/' . $smilies[$i]['smile_url'], 
			"CODE" => $smilies[$i]['code'],
