...
class display_chat_room_action extends action
{
	...
	function perform()
	{
		...
		if (user :: is_logged_in() && !chat_user :: is_logged_in())
			$chat_user->login(array('nickname' => user :: get_login()));
		
		if($chat_user_data = $chat_user->get_chat_user_data())
		{
			if($chat_user_data['chat_room_id'] != $chat_room_data['object_id'])
				$chat_system->enter_chat_room($chat_user_data['id'], $chat_room_data['object_id']);
		}
