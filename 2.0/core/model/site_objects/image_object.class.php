...
***********************************************************************************
*
* $Id: image_object.class.php 484 2004-02-20 10:01:54Z server $
*
***********************************************************************************/ 
...
class image_object extends media_object
{
	...
	function _update_generate_operation($variation, &$result)
	{
		...
		{
			$media_data = $this->_get_variation_media_data($this->get_attribute($variation . '_base_variation'));
	...
	{
		$image_id = $this->get_attribute('id');
	...
	function _get_variation_media_data($variation)
	{
		$image_id = $this->get_attribute('id');
	...
	function & fetch($params=array(), $sql_params=array())
	{
		...
		$sql = "SELECT iv.image_id, iv.media_id, iv.variation, iv.width, iv.height, m.size, m.mime_type, m.file_name, m.etag
				FROM image_variation iv, media m
	...
	function _check_result($result)//it's kil-l-ling me!
	{
		$image_variations = $this->_get_variations_ini_list();
