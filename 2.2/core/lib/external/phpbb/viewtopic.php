 ...
 *
 *   $Id: viewtopic.php,v 1.1 2003/07/04 18:33:45 pacha Exp $
 *
 *
 ***************************************************************************/
	...
	if ( $HTTP_GET_VARS['view'] == 'newest' )
	{
		...
		if ( isset($HTTP_COOKIE_VARS[$board_config['cookie_name'] . '_sid']) )
		{
			...
			if ( $session_id )
			{
				...
				$post_id = $row['post_id'];
				header($header_location . append_sid("viewtopic.php?" . POST_POST_URL . "=$post_id#$post_id", true));
				exit;
			}
		}

		header($header_location . append_sid("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id", true));
		exit;
	}
...
if( !$is_auth['auth_view'] || !$is_auth['auth_read'] )
{
	if ( !$userdata['session_logged_in'] )
	{
		...
		$header_location = ( @preg_match("/Microsoft|WebSTAR|Xitami/", getenv("SERVER_SOFTWARE")) ) ? "Refresh: 0; URL=" : "Location: ";
		header($header_location . "/root/login?redirect=". urlencode($_SERVER['PHP_SELF'] . "?$redirect"));
		exit;
	}
...
else
{
	if ( isset($HTTP_GET_VARS['unwatch']) )
	{
		...
		{
			$header_location = ( @preg_match("/Microsoft|WebSTAR|Xitami/", getenv("SERVER_SOFTWARE")) ) ? "Refresh: 0; URL=" : "Location: ";
			header($header_location . "/root/login?redirect=". urlencode($_SERVER['PHP_SELF'] . '?' . POST_TOPIC_URL . "=$topic_id&unwatch=topic"));
			exit;
		}
	}
	else
...
//
$pagination = ( $highlight_active ) ? generate_pagination("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id&amp;postdays=$post_days&amp;postorder=$post_order&amp;highlight=" . $HTTP_GET_VARS['highlight'], $total_replies, $board_config['posts_per_page'], $start) : generate_pagination("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id&amp;postdays=$post_days&amp;postorder=$post_order", $total_replies, $board_config['posts_per_page'], $start);
...
$template->assign_vars(array(
	...
	'U_VIEW_TOPIC' => append_sid("viewtopic.php?" . POST_TOPIC_URL . "=$topic_id&amp;start=$start&amp;postdays=$post_days&amp;postorder=$post_order&amp;highlight=" . $HTTP_GET_VARS['highlight']), 
...
if ( !empty($forum_topic_data['topic_vote']) )
{
	...
	if ( $vote_info = $db->sql_fetchrowset($result) )
	{
		...
		$template->assign_vars(array(
			...
			'S_POLL_ACTION' => append_sid("posting.php?" . POST_TOPIC_URL . "=$topic_id"))
		);
